...
local function Debug(text)
	naughty.notify({ title = text or type(text), timeout = 0 })
end
...
local testP = awful.popup({
	widget = {
		{
			{
				image = "/home/matt/.config/awesome/widgets/logout-menu/icons/lock.svg",
...
local progressBarPopup = awful.popup({
	...
	border_color = theme.colors.main,
	border_width = 2,
	placement = awful.placement.centered,
	shape = rounded(theme.dpi),
	visible = false,
...
local popupTimerSound = timer({ timeout = 3 })
popupTimerSound:connect_signal("timeout", function()
	progressBarPopup.visible = false
	popupTimerSound:stop()
end)

local function updatePopupSound()
	if not progressBarPopup.visible then
		progressBarPopup.visible = true
	end
	...
	progressBarPopup.widget:get_children_by_id("icon")[1].text = icon
	progressBarPopup.widget:get_children_by_id("progressbar")[1].value = vol / 100
end

local function updatePopupBrightness()
	if not progressBarPopup.visible then
		progressBarPopup.visible = true
	end
	local device = "@DEFAULT_SINK@"
...
local function lowerSound()
	awful.spawn("amixer set 'Master' 1%-")
	updatePopupSound()
	Timeout(popupTimerSound)
end

local function riseSound()
	awful.spawn("amixer set 'Master' 1%+")
	updatePopupSound()
	Timeout(popupTimerSound)
end
...
globalkeys = gears.table.join(
	...
	awful.key({ modkey }, "0", awful.tag.history.restore, { description = "go back", group = "tag" }),
	awful.key({ modkey }, "Escape", function()
		awful.spawn.with_shell("~/.config/awesome/widgets/logout-menu/rofi.sh")
	end, { description = "opens power mneu", group = "power" }),
	...
	end, { description = "Play / Pause", group = "fn keys" }),
