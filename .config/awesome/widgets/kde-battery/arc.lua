...
local function worker(user_args)
	...
	widget_with_icon:connect_signal("button::press", function(_, _, _, button)
		if button == 1 then
			if is_kdeconnect_open then
				-- Close the KDE Connect app
				awful.spawn("pkill kdeconnect-app")
				is_kdeconnect_open = false
			else
				-- Open the KDE Connect app
				awful.spawn("kdeconnect-app")
				is_kdeconnect_open = true
			end
		end
	end)
	...
	widget_with_icon:connect_signal("mouse::enter", function()
		-- Display percentage or status based on charging state
		local hover_text
		if status == "Disconnected" then
			hover_text = "Device is not connected"
		elseif status == "Charging" then
			hover_text = "Charging " .. charge .. "%"
		elseif status == "" then
			hover_text = charge .. "%"
		else
			hover_text = ""
